...
Try {
	...
	If(-not $appVendor) {
		[string]$appVendor = 'App Vendor Here'
	}
	If(-not $appName) {
		[string]$appName = 'App Name Here'
	}
	If(-not $appVersion) {
	...
	[string]$appScriptAuthor = 'Zach Choate'
	$deferTimes = 3
	##*===============================================
	## Variables: Install Titles (Only set here to override defaults set by the toolkit)
	[string]$installName = ''
	[string]$installTitle = "Deploy $appName"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		If(-not $StopProcess) { $StopProcess = "aclearlyfakeprocess" }
		Show-InstallationWelcome -CloseApps $StopProcess -CloseAppsCountdown 60 -AllowDefer -DeferTimes 3 -CheckDiskSpace -PersistPrompt
		...
		switch ($InstallerType){
			'msi' 	{
						Start-Process "C:\Windows\System32\msiexec.exe" -ArgumentList "/i `"$PSScriptRoot\Files\$InstallerFile`" $InstallerArgs" -Wait
			}
			'exe'	{
						Start-Process "$PSScriptRoot\Files\$InstallerFile" -ArgumentList $InstallerArgs -Wait
			}
		}
		...
		If (-not $useDefaultMsi) { Show-InstallationPrompt -Message "$appName was successfully deployed." -ButtonRightText 'OK' -Icon Information -NoWait }
	}
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		If(-not $StopProcess) {$StopProcess = "aclearlyfakeprocess" }
		Show-InstallationWelcome -CloseApps $StopProcess -CloseAppsCountdown 60
        ...
        foreach ($uninstall_string in $uninstall_strings) {
            ...
            if ($installertype -eq "exe") {
                ...
                $Logfile = "$logpath\uninstalllog-$appname-$(get-date -Format yyyyMMddTHHmmss).log"
